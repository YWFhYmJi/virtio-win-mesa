...
#define VN_CS_ENCODER_INITIALIZER_LOCAL(storage, size)                       \
   ...
   {                                                                         \
      ...
      .cur = storage, .end = (const unsigned char *)(storage) + (size),               \
   }
...
#define VN_CS_ENCODER_INITIALIZER(buf, size)                                 \
   ...
   {                                                                         \
      ...
      .buffer_count = 1, .buffer_max = 1, .current_buffer_size = size,       \
      .cur = (buf)->base, .end = (unsigned char *)((buf)->base) + (size),                       \
   }

#define VN_CS_DECODER_INITIALIZER(storage, size)                             \
   (struct vn_cs_decoder)                                                    \
   {                                                                         \
      .cur = storage, .end = ((unsigned char *)(const void *)(storage)) + (size),               \
   }
...
vn_cs_encoder_get_len(const struct vn_cs_encoder *enc)
{
   ...
   if (!cur_buf->committed_size)
      len += (unsigned char *)enc->cur - (unsigned char *)cur_buf->base;
   return len;
}
...
{
   if (unlikely(size > (unsigned char *)enc->end - (unsigned char *)enc->cur)) {
      if (!vn_cs_encoder_reserve_internal(enc, size)) {
         vn_cs_encoder_set_fatal(enc);
         return false;
      }
      assert(size <= (unsigned char *)enc->end - (unsigned char *)enc->cur);
   }
                    ...
                    size_t val_size)
{
   ...
   enc->cur = (unsigned char *)enc->cur + size;
}
                            ...
                            size_t val_size)
{
   ...
   if (unlikely(size > (unsigned char *)dec->end - (unsigned char *)dec->cur)) {
                   ...
                   size_t val_size)
{
   if (vn_cs_decoder_peek_internal(dec, size, val, val_size))
      dec->cur = (unsigned char *)dec->cur + size;
}
